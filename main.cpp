...
void at_recv_thread(void const* argument)
{
	while(true){
		...
		if(pStart == NULL){//handle reponds msg
...
void recv_detect_thread(void const* argument)
{
	while(true){
		if(pBufNext != 0){
			if(strstr(nb_uart_buf,AT_ZIPRECV) != NULL)
    ...
    while (true){
            ...
            if(notifySendID != invalidID) {
                if((systemTimer-notifySendCounter) >= SOCKET_RESEND_TIME)
								{
                   notify_msg_handle(notifySendID);  // >5s not receive resp msg, resend notify
                    ...
                    notify_msg_handle(notifyUpdateVersion);
									  ...
									  notify_msg_handle(notifyOTAResult);
								}
            }
            recv_msg_handle();
						...
						if(updateCodeFlag == true && (chargerInfo.status & CHARGER_STATUS_MASK) != charging ){ //update code while not charging
							...
							at_close_socket(AT_DEFAULT_SOCK_ID);
							update_code(); // begin to update code
						}
        }
